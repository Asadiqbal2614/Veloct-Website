...
export default function CareerForm({ initialJobs }: CareerFormProps) {
  const [jobs] = useState<JobPosting[]>(initialJobs);
  const [showForm, setShowForm] = useState(initialJobs.length === 0);
      ...
      <div className="max-w-3xl mx-auto px-4 py-12 sm:py-20">
        ...
        <div className="space-y-4 mb-10">
          ...
          <p className="text-slate-600 dark:text-gray-300 text-base">
            Take the next step in your career with VELOCT. Fill out the form
            below and our talent team will review your application.
          </p>
        </div>
        ...
        {jobs.length > 0 ? (
          ...
          <div className="bg-slate-100/70 backdrop-blur-sm border border-slate-200/80 dark:bg-white/5 dark:border-white/10 rounded-2xl p-8 sm:p-10 text-center mb-10">
            <Briefcase
              size={48}
              className="mx-auto mb-4 text-slate-300 dark:text-white/20"
            />
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
              Currently, we have no open vacancies.
            </h2>
            <p className="text-slate-600 dark:text-white/70 text-sm">
              However, we are always looking for great talent. Drop your details
              and CV below, and our team will contact you if a suitable position
              opens up.
            </p>
          </div>
        )}
        ...
        {showForm && (
          <div id="application-form">
              ...
              return job ? (
                <div className="mb-6 p-4 rounded-xl bg-[#FE7004]/10 border border-[#FE7004]/20">
                  <p className="text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <Briefcase size={14} className="text-[#FE7004]" />
                    Applying for:{" "}
                    <span className="text-[#FE7004] font-semibold">
                      {job.title}
                    </span>
                  </p>
                </div>
            ...
            <form
              ...
              className="bg-slate-100/70 backdrop-blur-sm border border-slate-200/80 dark:bg-white/5 dark:border-white/10 rounded-2xl p-6 sm:p-8 space-y-5"
            >
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label
                    htmlFor="fullName"
                    className="micro-label text-slate-500 dark:text-white/60 block mb-1.5"
                  >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    >
                      <option
                        value=""
                        disabled
                        className="bg-slate-50 dark:bg-[#00164A] text-slate-900 dark:text-white"
                      >
                        Day
                      </option>
                      {DAYS.map((d) => (
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5">
                    ...
                    <span className="text-xs text-slate-400 dark:text-white/30">
                      Max file size: 5MB &middot; .pdf, .doc, .docx
                    </span>
                  </div>
                </div>
                {uploadError && (
